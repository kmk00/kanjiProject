...
const selectOptions = () => {
  ...
  if (
    checkIfGradesInOptionSelected() == false ||
    options.selectFrom == "" ||
    options.selectTo == ""
  ) {
  ...
  filteredWordsByGrade.value = Object.values(words)
    .filter((word) => gradesToFilter.includes(word.grade))
    .map((word) => ({
      ...word,
      kanji: Object.keys(words).find((key) => words[key] === word),
    }));
...
const checkIfGradesInOptionSelected = () => {
  ...
  const anySelected = Object.values(options.grades).some((grade) =>
    Object.values(grade).some((status) => status === true)
  );
  return anySelected;
};
...
const initialiseQuiz = () => {
  ...
  if (
    quiz.value.numberOfQuestions < 4 ||
    quiz.value.numberOfQuestions > filteredWordsByGrade.value.length
  ) {
  ...
  quizToBeginning();
  quiz.value.questionsArray = pickQuestionsToQuiz(
    filteredWordsByGrade.value,
    quiz.value.numberOfQuestions
  );
};
...
<template>
  <div class="w-3/4 md:w-[600px] pb-3 mx-auto">
    <form v-if="filteredWordsByGrade.length == 0">
      <h1 class="mt-4 text-center text-xl p-2 md-text">Select Grade</h1>
      <div
        class="animate-fade-in flex md:flex-row md:gap-0 gap-2 flex-col md:items-center md:grid-cols-2"
      >
        <div
          class="flex self-center max-w-[200px] mb-3 md:mb-0 md:max-w-[193px] flex-wrap gap-2 items-center"
        >
          <GradeSelector
            v-for="i in 8"
            :key="i"
            :grade="i"
            :gradeStatus="gradesSelected[i - 1]"
            @selectGrade="selectGrade"
          />
        </div>
        <div class="grid grid-cols-2 grid-rows-2 gap-2 w-full">
          <select
            v-model="options.selectFrom"
            class="h-10 p-2 border rounded-md bg-custom-bg border-blue-gray-200 cursor-pointer"
            placeholder=""
          >
          ...
          <button
            @click.prevent="selectOptions"
            class="row-span-2 hover:bg-custom-orange/10 transition-all p-2 border rounded-md bg-custom-bg border-blue-gray-200 cursor-pointer"
          >
            Confirm
          </button>
          <select
            v-model="options.selectTo"
            class="h-10 p-2 border rounded-md bg-custom-bg border-blue-gray-200 cursor-pointer"
          >
    ...
    <div v-if="filteredWordsByGrade.length > 0" class="animate-fade-in mt-4">
      ...
      <p
        class="py-2"
        v-else-if="
          isGame && quiz.currentQuestionIndex < quiz.questionsArray.length
        "
      >
